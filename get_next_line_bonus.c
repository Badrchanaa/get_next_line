...
/*   Updated: 2023/11/28 17:46:59 by bchanaa          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	read_next_line(int fd, char **line,
							size_t line_size, size_t buff_size)
{
	char	*buff;
	ssize_t	r_bytes;
	...
	if (!buff)
		return (1);
	r_bytes = read(fd, buff, buff_size);
	while (r_bytes > 0)
	{
...
{
	static char	*cache_buff[1024];
	char		*line;
	char		*buff;
	size_t		line_size;
	int			is_err;

	if (fd < 0 || fd > 1024 || !BUFFER_SIZE_OK)
		return (NULL);
	buff = cache_buff[fd];
	cache_buff[fd] = NULL;
	line_size = 0;
	...
	if (!buff || !ft_strnchr(buff, '\n', line_size))
		is_err = read_next_line(fd, &buff, line_size, BUFFER_SIZE);
	if (!buff || is_err)
	{
		free(buff);
		return (NULL);
	}
	line = ft_get_line(buff, cache_buff + fd);
	free(buff);
	return (line);
}
